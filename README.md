...
- **预算可控**：持久化单任务预算、用量与成本。按剩余预算保守计算输入 Token 上限、截断 diff 和限制输出 Token；预算不足时跳过模型调用并保留规则评审。预算设为 0 表示不限制。由于不同 Provider 未必支持同名模型，预算控制不强制切换模型。
...
- **LLM 兼容性**：LLM Provider 使用 OpenAI Chat Completions 协议；Base URL 未填写 `/v1` 时会自动补全，兼容常见的 OpenAI 网关。
