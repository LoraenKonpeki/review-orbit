...
export function createSettingsRepository(db: Database) {
  return {
    async listProviders(): Promise<PublicProvider[]> {
      const rows = await db`SELECT id, kind, name, base_url, model, input_cny_per_million, output_cny_per_million, is_enabled, is_default, created_at, updated_at FROM provider_configs ORDER BY kind, name`;
      return rows.map((row) => ({ id: String(row.id), kind: row.kind as ProviderKind, name: row.name, baseUrl: row.base_url || undefined, model: row.model || undefined, inputCnyPerMillion: Number(row.input_cny_per_million), outputCnyPerMillion: Number(row.output_cny_per_million), isEnabled: row.is_enabled, isDefault: row.is_default, hasSecret: true, createdAt: row.created_at, updatedAt: row.updated_at }));
    },
